...
const (
	...
	defaultWriteTimeout =  100 * time.Second // used if context has no deadline
)
...
func (c *jsonCodec) writeJSON(ctx context.Context, v interface{}) error {
	...
	jsonMsg, ok := v.(*jsonrpcMessage)
	count := 0
	if ok {
		count = len(jsonMsg.Result)
	} else {
		jsonMsgArr, ok := v.([]*jsonrpcMessage)
		if ok {
			for _, msg := range jsonMsgArr {
				count += len(msg.Result)
			}
		}
	}
	log.Warn("writeJSON", "count", count)
	return c.encode(v)
}
